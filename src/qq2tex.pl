...
#                       17SEP2018; chenxing modification

# debug variable that allows display of output lines on terminal
$show=0;	# 0=false, 1=true

# get question file name from user
# print "\nqq2tex (28FEB00 version):\nEnter the question file name: "; # removed by chenxing 17SEP2018
$qqfile=$ARGV[0]; # get qqfile from argv, modified by chenxing 17SEP2018

open (QQFILE, $qqfile) || die "cannot open QQFILE: $!";

# give output file the extension .qu for TeX format question file
$outfile=$qqfile;
$dotloc=index($outfile,".");
if ($dotloc==-1){	# no extension on input file so tack on .qu
	$outfile=$outfile.".qu";
	}
else { # strip old extension, tack on .qu
	$outfile=substr($outfile,0,$dotloc).".qu";
	}
...
while(<QQFILE>){
	...
	if (index($line,")",length($line)-2)!=-1) { $line=substr($line,0,length($line)-1)." \n"};
	...
	# handle QQ
	if (/^QQ/){
		$line="\\QT{QQ \\bct".substr($line,2);	# add TEX code
		$QQcnt++;
	...
	while (($start=rindex($line,"**",$start)) !=-1) {
		...
		$m=substr($line,$lws+1,$start-$lws-1)."\^\{".substr($line,$start+2,$rws-$start-2);
		$line=$l.$m.$r;
		$start--;
	}
	...
	while (($start=rindex($line,"//",$start)) !=-1) {
		...
		# now insert the replacement characters
		$m=substr($line,$lws+1,$start-$lws-1)."_\{".substr($line,$start+2,$rws-$start-2);
		$line=$l.$m.$r;
		$start--;
	}
...
# removed by chenxing, this function moved to splitAline() in utils.py
#	# keep output line length <=80
#	if (length($line) >80) {
#		$line=substr($line,0,80)." \\ ".substr($line,80)  # tex will join lines
#	}
